...
ASSEMBLY_DEPS_JAR="" #proxcocoa-assembly-0.1-deps.jar
if [ -e "$FWDIR"/target/scala-$SCALA_VERSION/proxcocoa-assembly-*-deps.jar ]; then
  export ASSEMBLY_DEPS_JAR=`ls "$FWDIR"/target/scala-$SCALA_VERSION/proxcocoa-assembly*-deps.jar`
fi

if [[ -z $ASSEMBLY_DEPS_JAR ]]; then
  ASSEMBLY_JAR=""
  if [ -e "$FWDIR"/target/scala-$SCALA_VERSION/proxcocoa-assembly-*.jar ]; then
    export ASSEMBLY_JAR=`ls "$FWDIR"/target/scala-$SCALA_VERSION/proxcocoa-assembly*.jar`
  fi
